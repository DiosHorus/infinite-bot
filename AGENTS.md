...
- Audio: `setupAudioReceiver` suscribe por usuario que habla (`EndBehaviorType.AfterSilence` 500ms); los fragmentos solo se guardan si `isRecording` (`start`/`stop`, apagado por defecto tras `join`). Búfer circular limitado a 120s (`MAX_BYTES = 48000*2*2*120`). `clip` concatena el búfer → `ffmpeg -f s16le -ar 48000 -ac 2` → MP3, con cooldown de 30s por servidor; temporales de `clips/` se borran en `finally`.
