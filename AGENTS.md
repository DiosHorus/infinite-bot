...
- Motor Opus: requiere `@discordjs/opus` (nativo, dependencia **opcional**) u `opusscript` (respaldo puro-JS, en `package.json`); sonda `OPUS_OK` al arrancar + chequeo `opus` en `debug`. Sin motor: clips vacíos y `c!s` mudo. `join` usa `selfMute: false` (muteado no transmitiría sonidos) y `getGuildPlayer` resuscribe el player en cada uso (si no, mudo tras `leave`+`join`).
